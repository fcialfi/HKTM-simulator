...
.sec-item .value { white-space: nowrap; font-family: 'JetBrains Mono', monospace; font-size: 0.82rem; color: #8aa0bd; font-weight: 600; }

.kpi-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(175px, 1fr)); gap: 10px; }
.kpi { background: #0d1220; border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; padding: 0.7rem 0.9rem; }
.kpi .label { font-size: 0.66rem; color: #56698a; text-transform: uppercase; letter-spacing: 0.4px; }
.kpi .value { font-family: 'JetBrains Mono', monospace; font-size: 1.05rem; color: #e6edf5; font-weight: 600; margin-top: 4px; white-space: nowrap; }
.kpi .sub { font-size: 0.7rem; color: #56698a; margin-top: 2px; }
...
with st.sidebar:
    ...
    with st.expander("2 · Input format", expanded=True):
    ...
    with st.expander("3 · Channel coding", expanded=True):
        fec_rs = st.checkbox(
    ...
    with st.expander("4 · Payload & duration", expanded=True):
    ...
    if fec_rs or is_cadu_input:
        with st.expander("5 · Deterministic error injection", expanded=False):
    ...
    with st.expander("6 · Pulse shaping (RRC)", expanded=False):
    ...
    with st.expander("7 · Impairments & Doppler", expanded=bool(st.session_state.get("enable_impairments")
                                                                               or st.session_state.get("enable_doppler"))):
        ...
        st.markdown("**Channel (Doppler)**")
        enable_doppler = st.checkbox(
            "Enable Doppler", key="enable_doppler",
            help=(
...
def _panel_head(title, neutral=False):
    st.markdown(f'<div class="panel-head{" neutral" if neutral else ""}"><span class="dot"></span>'
...
<div class="hktm-header">
  <div>
    ...
    <div class="hktm-subtitle">CCSDS 131.0-B-5 &middot; test signal generation and recorded-pass analysis for RF-Catcher (TestTree)</div>
  </div>
...
MAIN_TABS = ["Signal generator", "Recording analysis"]
try:
    # Stateful tabs (newer Streamlit): lets "Apply to generator" switch back
    # to the generator tab.
    tab_gen, tab_rec = st.tabs(MAIN_TABS, key="main_tab", on_change="rerun")
except TypeError:
    tab_gen, tab_rec = st.tabs(MAIN_TABS)

# The analysis tab is built first so that an st.stop() in the generator's
# preview (on a chain error) can never hide it.
with tab_rec:
    with st.container(border=True):
        _panel_head("Recording")
        st.caption(
        ...
        with ra3:
            rec_duration = st.number_input(
                "Duration (s)", min_value=0.05, max_value=2.0, value=1.0, step=0.5, key="rec_duration",
                help="Slice analyzed (max 2 s: about 2 GB of memory per second of signal). 1 s is plenty. "
                     "Time depends only on this, not on the file size: about 15 s per second of signal, "
                     "plus ~15 s for decoding (always limited to the first ~40 CADUs).")
        with ra4:
        ...
        with rb2:
            rec_alpha = st.number_input("Matched filter roll-off", min_value=0.05, max_value=1.0, value=0.35,
                                        ...
                                        help="The satellite's RRC roll-off. 0.35 if unknown.")
        with rb3:
            st.write("")
    ...
    if rec is None:
        st.info("Enter a recording's path above and click **Analyze recording**: the results appear here.")
    else:
        ...
        with st.container(border=True):
            ...
            summary = [
                ("Symbol rate", f"{rec['rs']:,.1f} S/s".replace(",", " "),
                 f"{rec['rs_ppm']:+.1f} ppm vs {rec['rs_nominal']:,.0f}".replace(",", " ")),
        ...
        with st.container(border=True):
            _panel_head("Reproduce it in the generator", neutral=True)
            st.caption("Tick what to copy into the generator's settings, then click Apply: the generator "
                       "tab opens with those values set.")
            ac1, ac2, ac3, ac4 = st.columns(4)
            with ac1:
                st.checkbox(f"Symbol rate ({int(round(rec['rs'] / 50) * 50):,} S/s)".replace(",", " "),
                            value=True, key="apply_rs",
                            help="Rounded to 50 Hz so resampling to the recorder's rate stays exact.")
            with ac2:
            ...
            with bc1:
                st.button("Apply to generator", type="primary", width="stretch",
                          on_click=_apply_analysis_to_generator)
            with bc2:
...
with tab_gen:
    applied = st.session_state.pop("_applied_from_analysis", None)
    if applied:
        st.success("Applied from the recording analysis: " + ", ".join(applied) + ".")
